...
class Plot:
    ...
    * plot_essentials >>> plots the key plots all in one command (plot_mj, plot_postFreq, plot_conductance, plot_mnActivity)
    ...
    def plot_postSpikes(self, x_zoom: Optional[Tuple[float, float]]=None):
        ...
        for i in [0, 2]: # range(c.STDP_POST_NUM):
    ...
    def plot_postVolts(self, x_zoom: Optional[Tuple[float, float]]=None):
        '''Plots the spikes of the postsynaptic neuorns. This will plot every single spike that occurs, so it is only helpful when looking at a very small time window. Otherwise the spikes just form blocks of color.
        ...
        '''
        plt.figure(figsize=[25, 5])
        for i in [1, 3]: # range(c.STDP_POST_NUM):
            plt.plot(c.mj_t, c.stdp_data[i] * (1 - 0.05*i), linewidth='1', label=('Post: ' + str(i + 1)))
        if x_zoom is not None:
            plt.xlim(x_zoom)
        ...
        plt.savefig('./results/postVolts.png', dpi=self.dpi)

    def plot_postVoltsSpikes(self, x_zoom: Optional[Tuple[ float, float]]=None):
        ''' Plots the postsynaptic voltage and spike outputs in a single figure.
        '''
        ...
        plt.savefig('./results/postVoltsSpikes.png', dpi=self.dpi)
    ...
    def plot_all(self, x_zoom: Optional[Tuple[float, float]]=None):
        ''' Plot the all the plots!
        ...
        self.plot_conductance(x_zoom)
        self.plot_mnActivity(x_zoom)
        self.plot_postFreqIaCurr(x_zoom)
        self.plot_postSpikes(x_zoom)
        self.plot_postVolts(x_zoom)
