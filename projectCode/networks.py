...
def randConnections(pre_num, post_num, seed=None, g_min=1.50, g_max=4.00, inhibitoryConnections=False, inhibitoryConductance=1.0):
    if seed is not None:
        rng = Random(x=seed)
    else:
        rng = Random()
    ...
    if inhibitoryConnections == True:
        for post in range(post_num):
            matrix[pre_num + post, (pre_num + post_num - 1) - post] = inhibitoryConductance
